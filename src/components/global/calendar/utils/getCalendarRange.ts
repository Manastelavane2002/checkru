...
export const getCalendarRange = (id: CalendarRangeName): CalendarRange => {
  ...
  const processedBefore = new Date(before.getFullYear(), before.getMonth(), before.getDate());
  const processedAfter = new Date(after.getFullYear(), after.getMonth(), after.getDate());
  const processedNow = new Date(now.getFullYear(), now.getMonth(), now.getDate());
